<div id="carousel-container">
    <img src="" alt="" width="100%" height="400px" id="carousel-img">
    <div id="carousel-details">
        <div id="title-one">
            <h3 id="title-one-content"></h3>
        </div>
        <div id="title-two">
            <h1 id="title-two-content"></h1>
        </div>
        <div id="description">
            <p id="description-content"></p>
        </div>
        <div id="read-more-btn">
            <button type="button" id="read-more-btn-content"></button>
        </div>
    </div>
    <div id="image-selectors-box">
        <!-- Image selectors -->
    </div>
    <div id="side-contact-form-box">
        <p id="side-contact-form-text">Contact Us</p>
        <div id="contact-us-icon-box">
            <img src="<?php echo get_template_directory_uri(); ?>/assets/images/contact.png" alt="Contact Us">
        </div>
    </div>
</div>